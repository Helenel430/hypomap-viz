        ...
        tabPanel("Mean (expressing cells)", br(),
                 tags$p(style = "color:#555;font-size:12.5px;line-height:2;margin-bottom:12px",
                        HTML(sprintf(paste0(
                          ...
                          "<br><b>#Expressing</b> = cells within the cell type that carry &ge;1 detected count of the gene.",
                          "<br>The bar shows the gene's <b>absolute expression strength</b> on a fixed scale,",
                          " with a full bar marking the top 1%% of expression levels across every gene and cell",
                          " type (<b>%.2f</b>). These are log-normalized expression values, not raw counts.",
                          "<br>A near-full bar suggests strong absolute expression. For within-cell-type ranking,",
                          " see <b>Percentile (within cell type)</b> in the Table tab.",
